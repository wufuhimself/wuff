...
- **Keeper UI is hidden for leagues with no keeper slots** (2026-08-20) — `keeper_slots == 0` means "not a keeper league" (that is how the settings field is literally labelled), so no league page mentions keepers for one. `/league/<slug>/keepers` and `/keepers-board` **redirect to league settings** with a message rather than rendering a keeper-flavoured empty state; `/draft-history/<year>` forces `mode=all` (its live/keeper split is meaningless there, and a stale `?mode=keepers` bookmark would otherwise hit an empty table). Gated in templates: overview's "Keeper selection" link + "Set your keepers" draft CTA, both mock-draft pages' prose, draft-history's mode links/captions, "post-keeper board" wording on both draft-order pages, and the Scouting page's keeper example questions + its LLM prompt (`agent_reasoning._league_has_keepers()` — telling the model a redraft league's log holds keeper picks invites answers about data that cannot exist). **Two places keep keeper copy on purpose:** `league_settings.html` (where the number is set) and `welcome.html` (the public landing page — the keeper board is the headline feature being advertised, and it is not a league page; confirmed with the user 2026-08-20, don't strip it). ⚠️ **`league_overview.html` must use `overview_keeper_slots`, not `league_keeper_slots`** — the Sleeper/ESPN routes splat `_league_overview_ctx` *and* `_league_page_ctx` into `render_template`, so a shared key is a duplicate-keyword `TypeError`, and the Yahoo `/` route passes no `_league_page_ctx` at all, so that key would be undefined (falsy) there and would hide keeper links on frank-gore — the one league that has keepers. Verify a change here by rendering a redraft league (expect zero visible keeper phrases) **and** full-output-diffing frank-gore's pages (expect zero real text changes)
