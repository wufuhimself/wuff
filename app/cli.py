...
def parse_args() -> argparse.Namespace:
    ...
    yahoo_rankings_parser.add_argument('access_token', nargs='?', default=None,
        help='Yahoo OAuth access token (optional when saved locally)'
    )
    yahoo_rankings_parser.add_argument('--keeper', action='append',
        help='Keeper entries to exclude from draft rankings (playerId or playerName)'
    )
    ...
    fetch_league_rosters_parser = subparsers.add_parser(
        'fetch-league-rosters-mcp',
        help='Fetch all team rosters in your league via MCP and store as league snapshot',
    )
    fetch_league_rosters_parser.add_argument('--output', default=str(YAHOO_LEAGUE_ROSTERS_JSON),
    ...
    draft_history_parser.add_argument('--live-only', action='store_true',
        help='Exclude the last-2-rounds keeper slots (show only real draft-day picks)'
    )
    draft_history_parser.add_argument('--keepers-only', action='store_true',
        help='Show only the last-2-rounds keeper-slot picks for that year'
    )
    ...
    draft_order_parser = subparsers.add_parser(
        'draft-order',
        help='Show the snake draft order for the season after the given standings year (worst record picks first)',
    )
    ...
    keepers_board_parser = subparsers.add_parser(
        'keepers-board',
        help='Compute best-2-keepers for every team in the saved league roster snapshot, then write a post-keepers draft board CSV',
    )
    ...
    keepers_board_parser.add_argument('--output', default='data/processed/rankings_post_keepers.csv',
        help='Path to write the post-keepers CSV'
    )

    league_keys_parser = subparsers.add_parser(
        'list-league-keys',
        help='Resolve league_key mapping for historical seasons (game_key per year)',
    )
    league_keys_parser.add_argument('--seasons', nargs='+', type=int, default=[2020, 2021, 2022, 2023, 2024, 2025],
        help='Seasons to resolve'
    )
    ...
    position_round_parser = subparsers.add_parser(
        ...
        help='Analyze what position was drafted in a given round vs final standings rank',
    )
    ...
    combine_rankings_parser.add_argument('--output', default=None,
        help='Output file path (default: data/raw/rankings/rankings_combined.json)'
    )

    qb_adjust_parser = subparsers.add_parser(
        'apply-qb-adjustment',
        help="Standard 2026+ draft-forecasting adjustment: nudge the top-N QBs (by current ranking, "
             "from data/raw/rankings/yahoo_rankings.json) up to the overall pick where a QB of that "
             "rank has actually gone in this league's own draft history, then renumber the board. "
             "Also mirrors the result into rankings_combined.json (single-source) so "
             "keepers-board-export picks it up.",
    )
    ...
    qb_adjust_parser.add_argument('--years', nargs='+', type=int, default=None,
    ...
    subparsers.add_parser(
        'manager-keeper-history',
        help='Generate manager profiles with historical keeper data by comparing rosters year-to-year',
    )
    ...
    keepers_export_parser = subparsers.add_parser(
        ...
        help='Export per-team keeper recommendations and post-keepers draft board as CSVs',
    )
...
def load_saved_or_api_roster() -> tuple[List[YahooRosterPlayer], bool]:
...
def _cmd_migrate_data_layout(_args) -> None:
...
def _cmd_auth_server(_args) -> None:
...
def _cmd_yahoo_rankings(args) -> None:
    keepers = build_keepers(args.keeper)
    try:
        leagues = get_sync_leagues()
        if not leagues:
...
def _cmd_refresh_yahoo_rankings(_args) -> None:
    try:
        leagues = get_sync_leagues()
        if not leagues:
            print('No leagues found. Make sure MCP server is running on localhost:8000', file=sys.stderr)
...
def _cmd_import_rankings_csv(args) -> None:
...
def _cmd_apply_qb_adjustment(args) -> None:
...
def _cmd_import_rankings_pdf(args) -> None:
...
def _cmd_set_league_format(args) -> None:
...
def _cmd_show_league_format(_args) -> None:
    print(load_league_format().summary())



def _cmd_keeper_insight(args) -> None:
    roster_players, used_saved = load_saved_or_api_roster()
    if used_saved:
    ...
    if not league_format.keeper_cost_uses_draft_round:
        print(
            f'Keeper cost rule: none; keepers use the last draft slots and can be held for '
            f'up to {league_format.keeper_max_consecutive_seasons} consecutive seasons.'
        )
...
def _cmd_best_keepers(args) -> None:
    roster_players, used_saved = load_saved_or_api_roster()
    if used_saved:
        print(f'Loaded {len(roster_players)} roster players from data/raw/rosters/yahoo_roster.json')
    rankings = None
...
def _cmd_forecast_keepers(args) -> None:
    try:
    ...
    if not rankings:
        print('No saved rankings found. Run combine-rankings or refresh-yahoo-rankings first.', file=sys.stderr)
        sys.exit(1)
    roster_players, _used_saved = load_saved_or_api_roster()
    league_format = load_league_format()
    if args.teams != league_format.teams:
        league_format.teams = args.teams
...
def _cmd_yahoo_roster(_args) -> None:
    try:
        leagues = get_sync_leagues()
        if not leagues:
            print('No leagues found. Make sure MCP server is running on localhost:8000', file=sys.stderr)
...
def _cmd_save_roster(_args) -> None:
    try:
        leagues = get_sync_leagues()
        if not leagues:
            print('No leagues found. Make sure MCP server is running on localhost:8000', file=sys.stderr)
...
def _cmd_fetch_league_rosters_mcp(args) -> None:
    try:
        leagues = get_sync_leagues()
        if not leagues:
            print('No leagues found. Make sure MCP server is running on localhost:8000', file=sys.stderr)
...
def _cmd_saved_roster(_args) -> None:
    roster_players = load_roster()
...
def _cmd_yahoo_keepers(_args) -> None:
    try:
        leagues = get_sync_leagues()
        if not leagues:
            print('No leagues found. Make sure MCP server is running on localhost:8000', file=sys.stderr)
...
def _cmd_draft_history(args) -> None:
    years = load_draft_years()
...
def _cmd_draft_picks(args) -> None:
...
def _cmd_draft_order(args) -> None:
...
def _cmd_keepers_board(args) -> None:
    input_path = Path(args.input)
    try:
...
def _cmd_list_league_keys(args) -> None:
...
def _cmd_fetch_standings(args) -> None:
    access_token = resolve_access_token(None)
    try:
...
def _cmd_fetch_nfl_stats(args) -> None:
    try:
        seasons = None
        if args.seasons:
...
def _cmd_combine_rankings(args) -> None:
    try:
...
def _cmd_extract_keeper_history(_args) -> None:
    try:
...
def _cmd_manager_keeper_history(_args) -> None:
    try:
...
def _cmd_keeper_strategy(args) -> None:
    try:
...
def _cmd_backfill_standings_mcp(args) -> None:
    try:
...
def _cmd_keepers_board_export(args) -> None:
    try:
        league_rosters = json.loads(YAHOO_LEAGUE_ROSTERS_JSON.read_text())
    except FileNotFoundError:
...
_COMMAND_HANDLERS = {
    ...
    'auth': _cmd_auth,
    'auth-server': _cmd_auth_server,
    'token': _cmd_token,
    'refresh': _cmd_refresh,
    'rankings': _cmd_rankings,
    'yahoo-rankings': _cmd_yahoo_rankings,
    'refresh-yahoo-rankings': _cmd_refresh_yahoo_rankings,
    'import-rankings-csv': _cmd_import_rankings_csv,
    'apply-qb-adjustment': _cmd_apply_qb_adjustment,
    'import-rankings-pdf': _cmd_import_rankings_pdf,
    'set-league-format': _cmd_set_league_format,
    'show-league-format': _cmd_show_league_format,
    'keeper-insight': _cmd_keeper_insight,
    'best-keepers': _cmd_best_keepers,
    'forecast-keepers': _cmd_forecast_keepers,
    ...
    'roster-raw': _cmd_roster_raw,
    'set-lineup': _cmd_set_lineup,
    'draft-history': _cmd_draft_history,
    'draft-picks': _cmd_draft_picks,
    'standings': _cmd_standings,
    'draft-order': _cmd_draft_order,
