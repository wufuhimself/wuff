...
class LeagueFormat:
    ...
    @property
    def keeper_ineligible_round_set(self) -> set:
        return set(self.keeper_ineligible_rounds)
...
def league_format_from_payload(payload: object) -> LeagueFormat:
    """Build a LeagueFormat from a parsed JSON dict (league_settings.json or an inline
    'format' block in leagues.json). Missing/invalid fields fall back to defaults."""
    if not isinstance(payload, dict):
        return default_league_format()

    defaults = default_league_format()
    starters = payload.get('starters')
    if not isinstance(starters, dict):
        starters = defaults.starters
...
def load_league_format(path: Path = LEAGUE_SETTINGS_FILE) -> LeagueFormat:
    try:
        ...
        return default_league_format()
    return league_format_from_payload(payload)
