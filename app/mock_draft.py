...
from .paths import PROCESSED_DIR, RAW_STANDINGS_DIR, RAW_DRAFT_HISTORY_DIR
...
def load_adjusted_rankings(filepath: Optional[Path] = None) -> Tuple[Dict[str, Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    if not filepath.exists():
        from .paths import RAW_RANKINGS_DIR
...
def get_draft_order_2026(standings_year: int = 2025) -> List[str]:
    ...
    # Reverse to get worst-first order
    teams = [s.get('team', '') for s in reversed(standings)]
...
def build_position_need_scores(team: str, keeper_names: List[str], rankings_lookup: Dict) -> Dict[str, int]:
...
def get_team_round_preferences(team: str, round_num: int, all_rankings: List[Dict]) -> Dict[str, int]:
    """Analyze team's historical draft preferences for this round.
    Returns {position: preference_score}."""
    ...
    for year in range(2022, 2026):
        ...
        for pick in picks:
            if pick.get('team') == team and pick.get('round') == round_num:
                pos = pick.get('position', 'UNK')
                if pos != 'UNK':
                    position_counts[pos] += 1
                    total_picks += 1

    # Normalize to preference score (0-1)
    preferences = {}
    for pos in LEAGUE_STARTERS.keys():
        if total_picks > 0:
            preferences[pos] = position_counts.get(pos, 0) / total_picks
        else:
            preferences[pos] = 0.0
...
def score_pick(
    ...
    # Primary: inverse of rank (lower rank = higher score)
    ...
    team_pref = team_preferences.get(pos, 0)

    # Composite score: rank heavily weighted, position need and preference as tiebreaks
    score = rank_score + (pos_need * 0.5) + (team_pref * 2.0)
...
def simulate_draft(
    ...
    """Simulate 15-round draft. Returns list of picks: {round, pick_num, team, player_name, position, rank}."""

    # Track which players have been taken
    taken_players = set()
    taken_by_team = defaultdict(set)

    # Seed taken players with keepers
    for team, keeper_names in keeper_predictions.items():
        for keeper_name in keeper_names:
            key = keeper_name.lower().strip()
            taken_players.add(key)
            taken_by_team[team].add(key)
    ...
    for pick_num in range(1, 181):
        ...
        # Calculate position needs + team preferences
        position_needs = build_position_need_scores(team, keeper_predictions.get(team, []), rankings_lookup)
        team_preferences = get_team_round_preferences(team, round_num, rankings_all)
        ...
        for player in rankings_all:
            ...
            score = score_pick(player, team, round_num, position_needs, team_preferences)
        ...
        if best_player:
            ...
            taken_by_team[team].add(player_key)
            ...
            mock_picks.append({
                ...
                'position': best_player.get('position', 'UNK'),
...
def run_mock_draft() -> List[Dict[str, Any]]:
    ...
    mock_picks = simulate_draft(keeper_predictions, rankings_lookup, rankings_all, draft_order)
    return mock_picks
...
def export_mock_draft(picks: List[Dict[str, Any]], output_dir: Optional[Path] = None) -> Path:
    ...
    with open(csv_path, 'w', newline='') as f:
        fieldnames = ['round', 'pickInRound', 'team', 'playerName', 'position', 'rank', 'nflTeam']
